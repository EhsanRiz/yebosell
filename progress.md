...
**Engineering / safety:** no automated tests — a **Playwright smoke suite** over the core flows (checkout→track→docs, register→order→status→notify, admin drill-down) would catch regressions across the now-large single-file React+Babel pages.
