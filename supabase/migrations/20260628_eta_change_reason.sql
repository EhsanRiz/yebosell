-- A revision to the ETA must carry a reason; include it in the buyer message.
alter table public.orders add column if not exists eta_change_reason text;
...
begin
  ...
  -- ETA set / revised -> tell the buyer. A revision carries the seller's reason.
    ...
    values (new.id, new.seller_id, 'system', 'status',
            case when old.eta_date is null
              then 'Estimated ' || v_kind || ' date: ' || to_char(new.eta_date, 'Dy, DD Mon')
                   ...
                   || case when coalesce(new.eta_change_reason,'') <> '' then ' — ' || new.eta_change_reason else '' end
            end,
            jsonb_build_object('eta_date', new.eta_date, 'reason', new.eta_change_reason));
  end if;
