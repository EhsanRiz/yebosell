-- Internal fee-notification calls (send-push / send-sms) include an anon Bearer
-- JWT so they pass the edge gateway even when a function has verify_jwt=true
-- (send-sms was created with the dashboard default). The functions still gate on
-- the x-push-key shared secret. The anon key is public.
-- Prerequisite: private_config holds 'anon_key' = the project's public anon key
--   insert into private_config(key,value) values ('anon_key','<PROJECT_ANON_KEY>')
